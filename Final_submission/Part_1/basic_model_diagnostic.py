...
def simulate_on_policy_sample(
    ...
    """Simulate ergodic on-policy sample under the trained policy."""
    if n_paths is None:
        n_paths = tp.n_paths
...
def build_coverage_box_and_sample(
    ...
    """Build tight outer box from ergodic samples and draw uniform points."""
...
def edge_and_corner_points(
    ...
    """Construct edge and corner points on coverage box in (ln k, ln z)."""
...
def gh_robustness_check(
    ...
    """Compare GH-{compare_nodes} vs GH-base_nodes on a subsample."""
    N = len(k_np)
    n_sub = min(n_sub, N)
...
def final_test(params: BasicFinalTestParams) -> None:
    """Run comprehensive final evaluation with GH-based residuals."""
    print("\n========================")
    ...
    print_stats(f"On-policy test set (GH-{params.gh_nodes})", stats_ops, tol_list=params.tol_list)
    print_stats(f"Coverage test set (GH-{params.gh_nodes})", stats_cov, tol_list=params.tol_list)
    ...
    print(
        f"- GH-node robustness (vs {params.gh_nodes}): GH-15={'PASS' if gh15_ok else 'FAIL'}, "
...
def plot_deterministic_convergence(
    ...
    """Plot deterministic convergence paths of k_t and iota_t for z_t ≡ z_fixed."""
    ...
    for k0 in k0_list:
        ...
        for _ in range(T):
            ...
            iota_t = float(policy_iota(policy, k_tf, z_tf, training=False)[0].numpy())
            iotas.append(iota_t)
            k_next = (1.0 - delta + iota_t) * k_t
            k_t = max(1e-12, k_next)
            ks.append(k_t)
    ...
    axes[0].axhline(k_star_det, color="k", linestyle="--", label="k* (no adj. cost)")
...
def _build_k_z_grid(
    ...
    """Build rectangular (k, ln z) grid and flattened arrays for evaluation."""
    k_star_det = steady_state_k(mp.theta, mp.delta, mp.r)
...
def plot_policy_heatmap(
    ...
    """Plot 2D heatmap of policy function iota(k, z) = I/k."""
...
def plot_euler_residual_hist(
    ...
    """Plot histogram of Euler residuals g(k, z) on rectangular grid."""
    ...
    del KK, LL  # Only flattened values are used.
